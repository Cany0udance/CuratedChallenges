...
const vec3 COLOR = vec3(0.3, 0.6, 0.3);
...
void main() {
    ...
    st = vec2(st.x * u_screenSize.x / u_screenSize.y, st.y);
    float finalFog = 1.0;
    for (int i = 0; i < u_size; i++) {
        if (finalFog < 0.1) break; // Early exit if finalFog is small
        vec2 correctedMouse = vec2(u_positions[i].x / u_screenSize.x * (u_screenSize.x / u_screenSize.y), u_positions[i].y / u_screenSize.y);
        float distance = length(correctedMouse - st);
        float fogFactor = mix(1.0, 0.0, smoothstep(0.0, 0.2, distance));
